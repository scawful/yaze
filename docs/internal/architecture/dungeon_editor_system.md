...
- **Persistence coverage**: Tile objects, sprites, doors (marker + pointer table), room headers (14 bytes + message IDs), palettes, torches, pushable blocks, custom collision, chests, pot items, and regular dungeon entrances are written back and have focused regression coverage. Spawn-point writes fail closed pending a correct dedicated schema. The pit-damage table remains a protected preservation path because yaze does not expose a dedicated editor for that global table.
...
2. **Save Pipeline Follow-up**:
   ...
   - Add broader integration coverage for door/chest/pot/collision/write flows beyond the current unit and ROM-safety tests.
