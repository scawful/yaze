...
4. **Save**
   ...
   - `DungeonEditorV2::Save()` / `SaveRoom()` persist full managed-room state for objects, sprites, room headers, torches, pushable blocks, custom collision, chests, pot items, regular dungeon entrances, and the dedicated spawn-point model. Global aggregators run once across the managed-room set so later rooms do not clobber earlier chest/pot/collision writes.
