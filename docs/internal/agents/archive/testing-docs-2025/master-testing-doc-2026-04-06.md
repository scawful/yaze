# Testing Infrastructure - Master Documentation (ARCHIVED)

**Status**: ARCHIVE
**Archived**: 2026-09-16
**Superseded by**: [docs/internal/testing/README.md](../../../testing/README.md) (index) and
[docs/internal/testing/overview.md](../../../testing/overview.md) (canonical CI/test strategy)

> Archived because the commands and ownership below no longer match the repository.
> The single-binary invocations (`./build/bin/yaze_test --unit`, `--e2e`, `--benchmark`)
> and `ctest --preset dev` do not exist; tests build as per-suite binaries
> (`yaze_test_unit`, `yaze_test_integration`, `yaze_test_gui`, ...) and run through
> ctest labels. The `CLAUDE_*` owner handles and the `coordination-board.md` workflow
> were retired in favour of `docs/internal/agents/personas.md` and
> `scripts/agents/coord`. Kept for history only; do not edit.

**Original Owner**: CLAUDE_TEST_COORD
**Original Last Updated**: 2026-04-06
...
See [GH Actions Remote Guide](../utility-tools/gh-actions-remote.md) for details.
...
See [Coordination Board](../../coordination-board.md) for current status.
