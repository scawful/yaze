...
- `LoadAndBuildRoom` (`assets/asm/usdasm/bank_01.asm:$01873A`):
  ...
  7) Pushable blocks (`$7EF940`) and torches (`$7EFB40`) are drawn after the four passes. Pushable blocks honor stored bit 13 to select upper/BG1 or lower/BG2 before masking the encoded word to its tilemap position. Lightable torches are forced to upper/BG1 and mask their selector/lit bits before drawing.
