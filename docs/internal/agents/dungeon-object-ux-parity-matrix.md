...
| Context menu (objects) | Insert/Cut/Copy/Paste/Delete + layer/z-order; group adds Save As New Layout; single has Edit Graphics (disabled) | Insert menu (object/sprite/item/door), cut/copy/paste/duplicate/delete/delete-all + layer/z-order + shortcuts | Missing Save As New Layout + Edit Graphics (disabled); ordering parity | Add missing entries or explicitly defer; verify ordering | PARTIAL |
