...
void DungeonEditorIntegrationTest::SetUp() {
  ...
  ASSERT_TRUE(dungeon_editor_->Initialize().ok());
}
...
// MockRom implementation
absl::Status MockRom::LoadFromFile(const std::string& filename) {
  // Mock implementation - just return success
  return absl::OkStatus();
}

absl::Status MockRom::LoadZelda3() {
  // Mock implementation - just return success
  return absl::OkStatus();
}
...
void MockRom::SetMockData(const std::vector<uint8_t>& data) {
  mock_data_ = data;
  data_ = mock_data_.data();
  size_ = mock_data_.size();
}
