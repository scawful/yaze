...
class InteractionDelegationTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    auto& room = rooms_[0];
    room.AddTileObject(
        zelda3::RoomObject{0x55, 10, 10, 0x00, 0});  // Object at (10,10)
    room.AddTileObject(
        zelda3::RoomObject{0x02, 20, 20, 0x02, 0});  // Object at (20,20)
    room.AddTileObject(
        zelda3::RoomObject{0x03, 15, 15, 0x01, 0});  // Object at (15,15)
...
TEST_F(InteractionDelegationTest, CoordinatorModeTransitionsWork) {
  ...
  EXPECT_EQ(coordinator.GetCurrentMode(),
