...
class Spc700Test : public ::testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    // Set up callbacks to use mock audio RAM
    callbacks_.read = [this](uint16_t address) {
      return audioRAM.read(address);
    };
    callbacks_.write = [this](uint16_t address, uint8_t value) {
      audioRAM.write(address, value);
    };
    callbacks_.idle = [](bool waiting) {
      // Do nothing for idle callback
    };

    // Create SPC700 with the configured callbacks
    spc700 = std::make_unique<Spc700>(callbacks_);
