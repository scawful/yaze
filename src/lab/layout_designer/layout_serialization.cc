...
std::string DirToString(ImGuiDir dir) {
  switch (dir) {
    ...
    case ImGuiDir_Down:
      return "down";
    case ImGuiDir_COUNT:
      return "none";  // Unused, but handle for completeness
    default:
      return "none";
  }
}
...
DockNodeType StringToNodeType(const std::string& str) {
  if (str == "root")
    return DockNodeType::Root;
  if (str == "split")
    return DockNodeType::Split;
  if (str == "leaf")
...
std::string LayoutSerializer::SerializePanel(const LayoutPanel& panel) {
  return absl::StrFormat(
      ...
      EscapeJson(panel.panel_id), EscapeJson(panel.display_name),
      EscapeJson(panel.icon), panel.priority,
      panel.visible_by_default ? "true" : "false",
      panel.closable ? "true" : "false", panel.pinnable ? "true" : "false");
}
