...
class DockBuilderFacade {
 public:
  explicit DockBuilderFacade(ImGuiID dockspace_id)
...
bool ApplyLayoutToDockspace(LayoutDefinition* layout_def,
                            PanelManager* panel_manager, size_t session_id,
  ...
  std::function<void(DockNode*, ImGuiID)> build_tree = [&](DockNode* node,
                                                           ImGuiID node_id) {
    if (!node)
      return;

    if (node->IsSplit() && node->child_left && node->child_right) {
      DockSplitResult split =
          facade.Split(node_id, node->split_dir, node->split_ratio);
    ...
    for (const auto& panel : node->panels) {
      ...
      if (window_title.empty()) {
        LOG_WARN("LayoutDesigner", "Skipping panel '%s' (missing window title)",
...
void LayoutDesignerWindow::Draw() {
  ...
  if (ImGui::Begin(ICON_MD_DASHBOARD " Layout Designer", &is_open_,
    ...
    float canvas_width =
        ImGui::GetContentRegionAvail().x - properties_width - 8;
...
void LayoutDesignerWindow::DrawMenuBar() {
  if (ImGui::BeginMenuBar()) {
    if (ImGui::BeginMenu("File")) {
      ...
      if (ImGui::MenuItem(ICON_MD_SAVE " Save", "Ctrl+S", false,
                          current_layout_ != nullptr)) {
        // TODO(scawful): File dialog
        LOG_INFO("LayoutDesigner", "Save layout dialog");
      }
      if (ImGui::MenuItem(ICON_MD_SAVE_AS " Save As...", "Ctrl+Shift+S", false,
                          current_layout_ != nullptr)) {
        // TODO(scawful): File dialog
      ...
      if (ImGui::MenuItem(ICON_MD_DOWNLOAD " Export Code...", nullptr, false,
                          current_layout_ != nullptr)) {
    ...
    if (ImGui::BeginMenu("Edit")) {
      ...
      ImGui::Separator();
      if (ImGui::MenuItem(ICON_MD_DELETE " Delete Selected", "Del", false,
    ...
    if (ImGui::BeginMenu("Layout")) {
      if (ImGui::MenuItem(ICON_MD_PLAY_ARROW " Preview Layout", nullptr, false,
                          current_layout_ != nullptr)) {
        PreviewLayout();
      }
      if (ImGui::MenuItem(ICON_MD_CHECK " Validate", nullptr, false,
                          current_layout_ != nullptr)) {
        std::string error;
...
void LayoutDesignerWindow::DrawToolbar() {
  ...
  if (design_mode_ == DesignMode::PanelLayout) {
    if (current_layout_) {
      ImGui::Text("%s | %zu panels", current_layout_->name.c_str(),
                  current_layout_->GetAllPanels().size());
    }
  } else {
    if (current_panel_design_) {
      ImGui::Text("%s | %zu widgets", current_panel_design_->panel_name.c_str(),
...
void LayoutDesignerWindow::DrawPalette() {
  ...
  ImGui::SetNextItemWidth(-1);
  if (ImGui::BeginCombo("##category_filter",
                        selected_category_filter_.c_str())) {
    ...
    if (category_open) {
      for (const auto& panel : category_panels) {
        ...
        if (ImGui::IsItemHovered() && !panel.description.empty()) {
          ImGui::SetTooltip("%s\n\nID: %s\nPriority: %d",
                            panel.description.c_str(), panel.id.c_str(),
                            panel.priority);
        }
        ...
        if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_None)) {
          ...
          ImGui::SetDragDropPayload(
              kPanelPayloadType, panel.id.c_str(),
...
void LayoutDesignerWindow::DrawCanvas() {
  ...
  ImVec2 scaled_size = ImVec2(current_layout_->canvas_size.x * canvas_zoom_,
  ...
  for (float x_pos = 0; x_pos < scaled_size.x; x_pos += grid_step) {
    draw_list->AddLine(
        ...
        ImVec2(canvas_pos.x + x_pos, canvas_pos.y + scaled_size.y), grid_color);
  }
  for (float y_pos = 0; y_pos < scaled_size.y; y_pos += grid_step) {
    draw_list->AddLine(
        ImVec2(canvas_pos.x, canvas_pos.y + y_pos),
        ImVec2(canvas_pos.x + scaled_size.x, canvas_pos.y + y_pos), grid_color);
  }
...
void LayoutDesignerWindow::DrawDockNode(DockNode* node, const ImVec2& pos,
                                        const ImVec2& size) {
  if (!node)
    return;
  ...
  if (node->IsLeaf()) {
    ...
    for (const auto& panel : node->panels) {
      ...
      ImVec2 capsule_max =
          ImVec2(rect_max.x - panel_padding, capsule_pos.y + capsule_height);
      ImU32 capsule_fill =
          alpha_color(ImGui::GetColorU32(ImGuiCol_Header), 0.7f);
      ...
      draw_list->AddRect(capsule_min, capsule_max, capsule_border, 6.0f, 0,
                         1.5f);

      std::string label =
          absl::StrFormat("%s %s", panel.icon, panel.display_name);
    ...
    if (node->flags & ImGuiDockNodeFlags_NoTabBar)
      node_flags_str += "[NoTab] ";
    if (node->flags & ImGuiDockNodeFlags_HiddenTabBar)
      node_flags_str += "[HiddenTab] ";
    if (node->flags & ImGuiDockNodeFlags_NoCloseButton)
      node_flags_str += "[NoClose] ";
    if (node->flags & ImGuiDockNodeFlags_NoDockingOverMe)
      node_flags_str += "[NoDock] ";
    ...
    if (node->panels.empty()) {
      ...
      draw_list->AddText(ImVec2(pos.x + (size.x - text_size.x) / 2,
    ...
    if (node->split_dir == ImGuiDir_Left || node->split_dir == ImGuiDir_Right) {
      ...
      // Draw split line
      draw_list->AddLine(ImVec2(pos.x + split_x, pos.y),
      ...
      // Draw split line
      draw_list->AddLine(ImVec2(pos.x, pos.y + split_y),
...
void LayoutDesignerWindow::DrawPanelProperties(LayoutPanel* panel) {
  ...
  ImGui::CheckboxFlags("No Title Bar", &panel->flags,
  ...
  ImGui::CheckboxFlags("No Scrollbar", &panel->flags,
                       ImGuiWindowFlags_NoScrollbar);
  ImGui::CheckboxFlags("No Collapse", &panel->flags,
                       ImGuiWindowFlags_NoCollapse);
  ImGui::CheckboxFlags("No Background", &panel->flags,
...
void LayoutDesignerWindow::DrawNodeProperties(DockNode* node) {
  ...
  ImGui::CheckboxFlags("Auto-Hide Tab Bar", &node->flags,
                       ImGuiDockNodeFlags_AutoHideTabBar);
  ImGui::CheckboxFlags("No Docking Over Central", &node->flags,
                       ImGuiDockNodeFlags_NoDockingOverCentralNode);
  ImGui::CheckboxFlags("No Docking Split", &node->flags,
                       ImGuiDockNodeFlags_NoDockingSplit);
  ImGui::CheckboxFlags("No Resize", &node->flags, ImGuiDockNodeFlags_NoResize);
  ImGui::CheckboxFlags("No Undocking", &node->flags,
    ...
    if (design_mode_ == DesignMode::PanelLayout) {
      ...
      if (ImGui::BeginTabBar("WidgetCodeTabs")) {
        ...
        if (ImGui::BeginTabItem("Member Variables")) {
          if (current_panel_design_) {
            std::string code = WidgetCodeGenerator::GenerateMemberVariables(
...
std::string LayoutDesignerWindow::GenerateDockBuilderCode() const {
  ...
  absl::StrAppend(
      &code, absl::StrFormat(
                 ...
                 current_layout_->name, current_layout_->name));
      ...
      [&](DockNode* node, std::string parent_id_var) {
        if (!node || node->IsLeaf()) {
          ...
          if (node && node->flags != ImGuiDockNodeFlags_None) {
            absl::StrAppend(&code,
                            absl::StrFormat("  if (ImGuiDockNode* node = "
                                            "ImGui::DockBuilderGetNode(%s)) {\n"
                                            "    node->LocalFlags = %d;\n"
                                            "  }\n",
        ...
        // It's a split node
        std::string child_left_var =
            absl::StrFormat("dock_id_%d", node->child_left->node_id);
        std::string child_right_var =
            absl::StrFormat("dock_id_%d", node->child_right->node_id);

        // Assign IDs if not already assigned (simple counter for now)
        static int id_counter = 1000;
        if (node->child_left->node_id == 0)
          node->child_left->node_id = id_counter++;
        if (node->child_right->node_id == 0)
          node->child_right->node_id = id_counter++;
        ...
        switch (node->split_dir) {
          case ImGuiDir_Left:
            ...
            break;
          case ImGuiDir_Right:
            dir_str = "ImGuiDir_Right";
            break;
          case ImGuiDir_Up:
            dir_str = "ImGuiDir_Up";
            break;
          case ImGuiDir_Down:
            dir_str = "ImGuiDir_Down";
            break;
          default:
            dir_str = "ImGuiDir_Left";
            break;
        ...
        absl::StrAppend(
            ...
            absl::StrFormat(
                ...
                child_left_var, child_right_var, parent_id_var, dir_str,
                node->split_ratio, child_left_var, child_right_var));
        ...
        if (node->IsLeaf()) {
          for (const auto& panel : node->panels) {
            if (panel.flags != ImGuiWindowFlags_None) {
              absl::StrAppend(
                  &code,
                  absl::StrFormat("  // Note: Panel '%s' requires flags: %d\n",
                                  panel.panel_id, panel.flags));
            }
            absl::StrAppend(
                &code,
                absl::StrFormat("  ImGui::DockBuilderDockWindow(\"%s\", %s);\n",
...
std::string LayoutDesignerWindow::GenerateLayoutPresetCode() const {
  ...
  std::string code = absl::StrFormat(
      ...
      "  };\n"
      "}\n",
      current_layout_->name, current_layout_->name, current_layout_->name,
...
void LayoutDesignerWindow::DrawDockNodeTree(DockNode* node, int& node_index) {
  ...
  if (node->IsRoot()) {
    ...
    label = absl::StrFormat(
        "Split (%s, %.2f)",
        node->split_dir == ImGuiDir_Left || node->split_dir == ImGuiDir_Right
            ? "Horizontal"
            : "Vertical",
        node->split_ratio);
  } else {
  ...
  bool open = ImGui::TreeNodeEx((void*)(intptr_t)node_index, flags, "%s",
                                label.c_str());
  ...
  if (open) {
    if (node->IsSplit()) {
      ...
      for (size_t i = 0; i < node->panels.size(); i++) {
        ...
        ImGuiTreeNodeFlags panel_flags =
            ImGuiTreeNodeFlags_Leaf | ImGuiTreeNodeFlags_NoTreePushOnOpen;
        if (selected_panel_ == &panel) {
          panel_flags |= ImGuiTreeNodeFlags_Selected;
        }

        ImGui::TreeNodeEx((void*)(intptr_t)node_index, panel_flags, "%s %s",
                          panel.icon.c_str(), panel.display_name.c_str());
...
void LayoutDesignerWindow::DeleteNode(DockNode* node_to_delete) {
  if (!current_layout_ || !current_layout_->root ||
      node_to_delete == current_layout_->root.get()) {
  ...
  std::function<bool(DockNode*)> find_parent = [&](DockNode* n) -> bool {
    ...
    if (find_parent(n->child_left.get()))
      return true;
    if (find_parent(n->child_right.get()))
      return true;
...
void LayoutDesignerWindow::DeletePanel(LayoutPanel* panel_to_delete) {
  ...
  std::function<bool(DockNode*)> find_and_delete = [&](DockNode* n) -> bool {
    ...
    if (n->IsLeaf()) {
      ...
      if (find_and_delete(n->child_left.get()))
        return true;
      if (find_and_delete(n->child_right.get()))
        return true;
    }
    return false;
  };
...
LayoutDesignerWindow::GetAvailablePanels() const {
  ...
  if (panel_manager_) {
    ...
    panel_cache_.push_back({"dungeon.room_selector", "Room List", ICON_MD_LIST,
                            "Dungeon", "Browse and select dungeon rooms", 20});
    ...
    panel_cache_.push_back({"graphics.sprite_editor", "Sprite Editor",
                            ICON_MD_PERSON, "Graphics", "Edit sprite graphics",
                            20});
  }
...
bool LayoutDesignerWindow::MatchesSearchFilter(
    const PalettePanel& panel) const {
...
void LayoutDesignerWindow::LoadLayout(const std::string& filepath) {
  ...
  if (result.ok()) {
    current_layout_ =
        std::make_unique<LayoutDefinition>(std::move(result.value()));
    selected_panel_ = nullptr;
    selected_node_ = nullptr;
...
void LayoutDesignerWindow::PreviewLayout() {
  ...
  if (!layout_manager_ || !panel_manager_) {
    LOG_WARN("LayoutDesigner",
             "Preview requires LayoutManager and PanelManager");
    return;
  }
...
void LayoutDesignerWindow::DrawDropZones(const ImVec2& pos, const ImVec2& size,
  ...
  LOG_INFO("DragDrop", "Drawing drop zones, mouse zone: %d",
  ...
  std::vector<DropZone> zones = {
      // Left zone
      {rect_min, ImVec2(rect_min.x + size.x * zone_size, rect_max.y),
       ImGuiDir_Left},
      // Right zone
      {ImVec2(rect_max.x - size.x * zone_size, rect_min.y), rect_max,
       ImGuiDir_Right},
      // Top zone
      {rect_min, ImVec2(rect_max.x, rect_min.y + size.y * zone_size),
       ImGuiDir_Up},
      // Bottom zone
      {ImVec2(rect_min.x, rect_max.y - size.y * zone_size), rect_max,
       ImGuiDir_Down},
      // Center zone
  ...
  for (const auto& drop_zone : zones) {
    ...
    ImU32 color = is_hovered ? alpha_color(base_zone, 0.8f)
    ...
    draw_list->AddRect(drop_zone.min, drop_zone.max,
                       ImGui::GetColorU32(ImGuiCol_HeaderActive), 4.0f, 0,
                       1.0f);
    ...
    if (is_hovered) {
      ...
      LOG_INFO("DragDrop", "✓ Drop target set: zone=%d",
               static_cast<int>(zone));
      ...
      ImVec2 text_pos =
          ImVec2((drop_zone.min.x + drop_zone.max.x - text_size.x) / 2,
...
ImGuiDir LayoutDesignerWindow::GetDropZone(const ImVec2& mouse_pos,
  ...
  ImVec2 relative_pos =
      ImVec2(mouse_pos.x - rect_min.x, mouse_pos.y - rect_min.y);
...
void LayoutDesignerWindow::AddPanelToTarget(const PalettePanel& panel) {
  ...
  LOG_INFO("DragDrop", "Creating panel: %s, target node type: %s",
           new_panel.display_name.c_str(), target->IsLeaf() ? "Leaf" : "Split");
  ...
  if (target->IsLeaf() && target->panels.empty()) {
    ...
    current_layout_->Touch();
    ResetDropState();
    LOG_INFO("LayoutDesigner", "✓ Added panel '%s' to leaf/root",
             panel.name.c_str());
    return;
  }
  ...
  float split_ratio =
      (drop_direction_ == ImGuiDir_Right || drop_direction_ == ImGuiDir_Down)
          ? 0.7f
          : 0.3f;
  ...
  DockNode* child =
      (drop_direction_ == ImGuiDir_Left || drop_direction_ == ImGuiDir_Up)
          ? target->child_left.get()
          : target->child_right.get();
  if (child) {
    child->AddPanel(new_panel);
    LOG_INFO("LayoutDesigner", "✓ Added panel to split child (dir=%d)",
             drop_direction_);
  } else {
...
void LayoutDesignerWindow::DrawWidgetPalette() {
  ...
  ImGui::InputTextWithHint(
      "##widget_search", ICON_MD_SEARCH " Search widgets...",
      widget_search_filter_, sizeof(widget_search_filter_));

  // Category filter
  ImGui::SetNextItemWidth(-1);
  const char* categories[] = {"All",        "Basic",  "Layout",
                              "Containers", "Tables", "Custom"};
  if (ImGui::BeginCombo("##widget_category",
                        selected_widget_category_.c_str())) {
  ...
  for (const auto& item : widgets) {
    // Category filter
    if (selected_widget_category_ != "All" &&
        item.category != selected_widget_category_) {
      continue;
    }
    ...
    if (item.category != current_category) {
      ...
      ImGui::TextColored(ImVec4(0.5f, 0.8f, 1.0f, 1.0f), "%s",
                         current_category.c_str());
      ImGui::Separator();
    }

    // Widget item
    ImGui::PushID(static_cast<int>(item.type));
    std::string label = absl::StrFormat("%s %s", GetWidgetTypeIcon(item.type),
                                        GetWidgetTypeName(item.type));

    if (ImGui::Selectable(label.c_str(), false, 0, ImVec2(0, 28))) {
      LOG_INFO("LayoutDesigner", "Selected widget: %s",
               GetWidgetTypeName(item.type));
    }
...
void LayoutDesignerWindow::DrawWidgetCanvas() {
  ...
  if (!current_panel_design_) {
    ...
    ImGui::Spacing();
    ImGui::TextWrapped(
        "Create a new panel design or select a panel from the Panel Layout "
        "mode.");
  ...
  for (const auto& widget : current_panel_design_->widgets) {
    ...
    // Check if clicked
    ImVec2 mouse_pos = ImGui::GetMousePos();
    if (mouse_pos.x >= widget_pos.x &&
        mouse_pos.x <= widget_pos.x + widget_size.x &&
        mouse_pos.y >= widget_pos.y &&
        mouse_pos.y <= widget_pos.y + widget_size.y) {
  ...
  if (ImGui::BeginDragDropTarget()) {
    ...
    if (const ImGuiPayload* payload =
      ...
      LOG_INFO("LayoutDesigner", "Added widget: %s",
               GetWidgetTypeName(*widget_type));
    }
...
void LayoutDesignerWindow::DrawWidgetProperties() {
  ...
  strncpy(tooltip_buffer, selected_widget_->tooltip.c_str(),
  ...
  strncpy(callback_buffer, selected_widget_->callback_name.c_str(),
  ...
  if (ImGui::Button("Delete Widget")) {
    ...
    LOG_INFO("LayoutDesigner", "Delete widget: %s",
             selected_widget_->id.c_str());
  }
}
...
void LayoutDesignerWindow::DrawWidgetTree() {
  ...
  for (const auto& widget : current_panel_design_->widgets) {
    ...
    if (ImGui::Selectable(
            absl::StrFormat("%s %s", GetWidgetTypeIcon(widget->type),
                            widget->id)
                .c_str(),
            is_selected)) {
      selected_widget_ = widget.get();
...
void LayoutDesignerWindow::DrawWidgetCodePreview() {
  ...
  std::string code =
      WidgetCodeGenerator::GeneratePanelDrawMethod(*current_panel_design_);
  ImGui::TextUnformatted(code.c_str());
}
