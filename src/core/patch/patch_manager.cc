...
absl::Status PatchManager::LoadPatches(const std::string& patches_dir) {
  ...
  std::vector<std::string> default_folders = {"Misc", "Hex Edits", "Sprites",
                                              "Items", "Npcs"};
...
absl::Status PatchManager::ApplyEnabledPatches(Rom* rom) {
...
#if defined(YAZE_WITH_Z3DK)
  // z3dk backend path — equivalent flow, just a different assembler.
  Z3dkWrapper z3dk;
  if (auto init_status = z3dk.Initialize(); !init_status.ok()) {
    std::filesystem::remove(temp_path);
    return init_status;
  }

  auto& rom_data = rom->mutable_vector();
  Z3dkAssembleOptions options = z3dk_options_;
  if (std::find(options.include_paths.begin(), options.include_paths.end(),
                patches_directory_) == options.include_paths.end()) {
    options.include_paths.push_back(patches_directory_);
  }
  auto result_or_error = z3dk.ApplyPatch(temp_path, rom_data, options);
  ...
  auto result_or_error =
      asar.ApplyPatch(temp_path, rom_data, {patches_directory_});
  ...
  return absl::UnimplementedError(
      "No assembler backend enabled. Build with YAZE_ENABLE_ASAR=ON or "
      "YAZE_ENABLE_Z3DK=ON.");
#endif
}
...
absl::Status PatchManager::GenerateCombinedPatch(
  ...
  combined << "; Do not edit this file directly - modify individual patches "
              "instead.\n";
  combined << "; "
              "================================================================"
              "=============\n\n";
...
absl::Status PatchManager::SaveAllPatches() {
  ...
  for (const auto& patch : patches_) {
    ...
    if (!status.ok()) {
      errors.push_back(absl::StrCat(patch->filename(), ": ", status.message()));
    }
  }

  if (!errors.empty()) {
    return absl::InternalError(absl::StrCat("Failed to save some patches:\n",
                                            absl::StrJoin(errors, "\n")));
  }
...
absl::Status PatchManager::RemovePatchFolder(const std::string& folder_name) {
  ...
  if (it == folders_.end()) {
    return absl::NotFoundError(absl::StrCat("Folder not found: ", folder_name));
  }
...
absl::Status PatchManager::AddPatchFile(const std::string& source_path,
  ...
  fs::path target_path =
      fs::path(patches_directory_) / target_folder / filename;
