...
VersionManager::VersionManager(project::YazeProject* project) : project_(project) {}

bool VersionManager::IsGitInitialized() const {
  if (!project_ || project_->git_repository.empty()) return false;
...
absl::Status VersionManager::InitializeGit() {
  if (!project_) return absl::InvalidArgumentError("No project context");
  ...
  if (IsGitInitialized()) return absl::OkStatus();

  return GitInit();
}

absl::StatusOr<SnapshotResult> VersionManager::CreateSnapshot(const std::string& message) {
  if (!project_) return absl::InvalidArgumentError("No project context");
  ...
  if (backup_status.ok()) {
    ...
    LOG_ERROR("VersionManager", "Failed to backup ROM: %s", backup_status.ToString().c_str());
  }
...
std::vector<std::string> VersionManager::GetHistory(int limit) const {
  std::string cmd = absl::StrFormat("git log -n %d --pretty=format:\"%%h %%s\"", limit);
...
absl::Status VersionManager::BackupRomArtifact(const std::string& timestamp_str) {
  if (project_->output_folder.empty() || project_->build_target.empty()) {
    return absl::FailedPreconditionError("Project output folder or build target not defined");
  }
  ...
  if (!fs::exists(abs_rom_path)) {
    return absl::NotFoundError(absl::StrFormat("Built ROM not found at %s", abs_rom_path.string()));
  }
  ...
  if (ec) return absl::InternalError("Could not create backup directory");

  std::string filename = absl::StrFormat("rom_%s.sfc", timestamp_str);
  fs::path backup_path = backup_dir / filename;

  // Copy file
  fs::copy_file(abs_rom_path, backup_path, fs::copy_options::overwrite_existing, ec);
  if (ec) {
    return absl::InternalError(absl::StrFormat("Failed to copy ROM: %s", ec.message()));
  }
