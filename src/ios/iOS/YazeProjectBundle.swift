    ...
    do {
      ...
      if fileManager.fileExists(atPath: settingsURL.path) {
        try fileManager.copyItem(
          at: settingsURL, to: bundleURL.appendingPathComponent("settings.json"))
      }
      ...
      let bundledRomURL = bundleURL.appendingPathComponent("rom")
      let romChecksum: String?
      if fileManager.fileExists(atPath: bundledRomURL.path) {
        romChecksum = try YazeRomChecksum.sha1Hex(contentsOf: bundledRomURL)
      } else {
        romChecksum = nil
      }
