...
ftxui::Component AsarPatchComponent::Render() {
  ...
  auto patch_file_input =
      Input(&state->patch_file, "Assembly patch file (.asm)");
  ...
  auto show_symbols_button = Button(
      "Show Symbols", [state] { state->show_symbols = !state->show_symbols; });
  ...
  return Renderer(container, [patch_file_input, apply_button,
                              show_symbols_button, back_button, state] {
