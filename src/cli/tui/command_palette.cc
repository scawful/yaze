...
Component CommandPaletteComponent::Render() {
  ...
  auto menu = Renderer([state] {
    ...
    if (state->query.empty()) {
      ...
      std::sort(filtered_idx.begin(), filtered_idx.end(),
                [](int a, int b) { return cmds[a].score > cmds[b].score; });
    }
    Elements items;
  ...
  return Renderer(
             container,
             [container, search_input, menu, back_btn, state] {
               std::vector<int> filtered_idx;
               if (state->query.empty()) {
                 for (size_t i = 0; i < cmds.size(); ++i) {
                   filtered_idx.push_back(i);
               ...
               if (state->selected < static_cast<int>(filtered_idx.size())) {
                 ...
                 details = vbox({
                     ...
                     separator(),
                     text("Usage: " + cmds[idx].name + " " + cmds[idx].usage) |
                         color(Color::Cyan),
                 });
               }
               ...
               return vbox({
                          ...
                          separator(),
                          text(state->status_msg) | center |
                              (state->status_msg.find("✓") != 0
                                   ? color(Color::Green)
                                   : color(Color::Red)),
                          text("↑↓: Navigate | Enter: Execute | Esc: Back") |
                              center | dim,
                      }) |
                      border | flex;
             }) |
         CatchEvent([state, execute_command](const Event& e) {
           ...
           if (e == Event::ArrowDown) {
             ...
             if (state->selected < static_cast<int>(filtered_idx.size()) - 1)
               state->selected++;
             return true;
           }
           return false;
