...
struct GeminiConfig {
  ...
  mutable std::string
      system_instruction;              // Mutable to allow lazy initialization
  bool use_enhanced_prompting = true;  // Enable few-shot examples
  bool use_function_calling =
      true;  // Use native Gemini function calling (enabled by default for 2.0+)
  std::string prompt_version =
      "v3";              // Which prompt file to use (default, v2, v3, etc.)
...
class GeminiAIService : public AIService {
 public:
  ...
  absl::StatusOr<AgentResponse> GenerateMultimodalResponse(const std::string&,
                                                           const std::string&) {
    return absl::FailedPreconditionError("Gemini AI runtime is disabled");
  }
  absl::StatusOr<std::vector<ModelInfo>> ListAvailableModels() override {
