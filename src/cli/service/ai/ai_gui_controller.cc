...
absl::StatusOr<ControlResult> AIGUIController::ExecuteActions(
  ...
  for (const auto& action : actions) {
    ...
    while (retry_count < config_.max_retries_per_action && !action_succeeded) {
      ...
      auto execute_result = ExecuteSingleAction(
          current_action, config_.enable_vision_verification);
      ...
      if (execute_result->action_successful) {
        ...
        auto refinement =
            vision_refiner_->RefineAction(current_action, *execute_result);

        if (!refinement.ok()) {
          result.error_message = absl::StrCat("Failed to refine action: ",
        ...
        if (refinement->needs_different_approach) {
          result.error_message = absl::StrCat(
              "Action requires different approach: ", refinement->reasoning);
          return result;
        }
  ...
  if (final_screenshot.ok()) {
    ...
    auto final_analysis = vision_refiner_->AnalyzeScreenshot(
        *final_screenshot, "Verify all actions completed successfully");
  ...
  if (action.type == AIActionType::kClickButton) {
    ...
    if (!result->success) {
      return absl::InternalError(absl::StrCat(
          "Assert action failed: ", result->message, " (expected: ",
          result->expected_value, ", actual: ", result->actual_value, ")"));
    }
    ...
    // Set the map ID
    auto type_result =
        gui_client_->Type("input:Map ID", std::to_string(map_id), true);
    ...
    return absl::UnimplementedError(absl::StrCat(
        "Action type not implemented: ", static_cast<int>(action.type)));
  }
#else
