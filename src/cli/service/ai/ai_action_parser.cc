...
absl::StatusOr<std::vector<AIAction>> AIActionParser::ParseCommand(
  ...
  if (absl::StrContains(cmd_lower, "click") ||
      absl::StrContains(cmd_lower, "press")) {
    std::regex button_pattern(R"((save|load|export|import|open)\s+(\w+))",
                              std::regex::icase);
    std::smatch match;
  ...
  switch (action.type) {
    ...
    case AIActionType::kPlaceTile: {
      ...
      if (x_it != action.parameters.end() && y_it != action.parameters.end()) {
        return absl::StrCat("Place tile at position (", x_it->second, ", ",
                            y_it->second, ")");
      }
      return "Place tile";
    }
...
bool AIActionParser::MatchesPlaceTilePattern(
  ...
  // Extract tile ID
  std::regex tile_pattern(R"(tile\s+(?:id\s+)?(0x[0-9a-fA-F]+|\d+))",
                          std::regex::icase);
  std::smatch match;
  ...
  std::regex map_pattern(R"((?:map|overworld)\s+(?:id\s+)?(\d+))",
                         std::regex::icase);
...
bool AIActionParser::MatchesOpenEditorPattern(
  ...
  if (absl::StrContains(cmd_lower, "tile16") ||
      absl::StrContains(cmd_lower, "tile 16")) {
