...
struct OllamaConfig {
  ...
  std::string model =
      "qwen2.5-coder:0.5b";            // Lightweight default with tool-calling
...
class OllamaAIService : public AIService {
 public:
  ...
  std::string GetProviderName() const override { return kProviderOllama; }
...
class OllamaAIService : public AIService {
 public:
  ...
  absl::StatusOr<AgentResponse> GenerateResponse(const std::string&) override {
    return absl::FailedPreconditionError("Ollama AI runtime is disabled");
  }
  absl::StatusOr<AgentResponse> GenerateResponse(
