...
ResourceSchema MakePaletteSchema() {
  ...
  schema.description =
      "Palette manipulation commands covering export, import, and color "
      "editing.";

  ResourceAction export_action;
  export_action.name = "export";
  export_action.synopsis =
      "z3ed palette export --group <group> --id <id> --to <file>";
  export_action.stability = "experimental";
  export_action.arguments = {
      ResourceArgument{"--group", "integer", true, "Palette group id (0-31)."},
      ResourceArgument{"--id", "integer", true,
                       "Palette index inside the group."},
      ResourceArgument{"--to", "path", true,
                       "Destination file path for binary export."},
  };
...
ResourceSchema MakeRomSchema() {
  ...
  info_action.arguments = {
      ResourceArgument{"--rom", "path", true,
                       "Path to ROM file configured via global flag."},
  };
  info_action.effects = {
      "Reads ROM from disk and displays basic information (title, size, "
      "filename)."};
  info_action.returns = {{"title", "string", "ROM internal title from header."},
  ...
  diff_action.effects = {
      "Reads two ROM images, compares bytes, and streams differences to "
      "stdout."};
  diff_action.returns = {
  ...
  generate_action.arguments = {
      ...
      ResourceArgument{"golden_file", "path", true,
                       "Output path for golden image."},
  };
...
ResourceSchema MakePatchSchema() {
  ...
  asar_action.effects = {
      "Invokes Asar against the active ROM buffer and applies assembled "
      "changes."};
  asar_action.returns = {
  ...
  create_action.effects = {
      "Compares source and target images to synthesize a distributable BPS "
      "patch."};
...
ResourceSchema MakeOverworldSchema() {
  ...
  get_tile.arguments = {
      ResourceArgument{"--map", "integer", true,
                       "Overworld map identifier (0-63)."},
      ResourceArgument{"--x", "integer", true, "Tile x coordinate."},
      ResourceArgument{"--y", "integer", true, "Tile y coordinate."},
  };
  get_tile.returns = {
      {"tile", "integer", "Tile id located at the supplied coordinates."}};
...
ResourceSchema MakeDungeonSchema() {
  ...
  export_action.effects = {
      "Loads the active ROM via --rom and prints metadata for the requested "
      "room."};
  export_action.returns = {{"metadata", "object",
                            "Structured room summary including blockset, "
                            "spriteset, palette, and layout."}};
  ...
  list_objects_action.arguments = {
      ResourceArgument{
          "room_id", "integer", true,
          "Dungeon room identifier whose objects should be listed."},
  };
  list_objects_action.effects = {
      "Streams parsed dungeon object records for the requested room to "
      "stdout."};
...
ResourceSchema MakeAgentSchema() {
  ...
  schema.description =
      "Agent workflow helpers including planning, diffing, listing, and schema "
      "discovery.";
  ...
  diff_action.returns = {
      ...
      {"metadata", "object",
       "Proposal metadata including status and timestamps."}};
...
ResourceCatalog::ResourceCatalog()
    : resources_({MakeRomSchema(), MakePatchSchema(), MakePaletteSchema(),
                  MakeOverworldSchema(), MakeDungeonSchema(),
                  MakeAgentSchema()}) {}

absl::StatusOr<ResourceSchema> ResourceCatalog::GetResource(
...
const std::vector<ResourceSchema>& ResourceCatalog::AllResources() const {
  return resources_;
}

std::string ResourceCatalog::SerializeResource(
    const ResourceSchema& schema) const {
  return SerializeResources({schema});
}

std::string ResourceCatalog::SerializeResources(
    ...
    for (const auto& action : resource.actions) {
      ...
      for (const auto& ret : action.returns) {
        return_entries.push_back(absl::StrCat(
            "{\"field\":\"", EscapeJson(ret.field), "\",\"type\":\"",
            EscapeJson(ret.type), "\",\"description\":\"",
            EscapeJson(ret.description), "\"}"));
      }
      action_entries.push_back(absl::StrCat(
          "{\"name\":\"", EscapeJson(action.name), "\",\"synopsis\":\"",
          EscapeJson(action.synopsis), "\",\"stability\":\"",
          EscapeJson(action.stability), "\",\"arguments\":[",
          absl::StrJoin(arg_entries, ","), "],", "\"effects\":[",
          absl::StrJoin(effect_entries, ","), "],", "\"returns\":[",
          absl::StrJoin(return_entries, ","), "]}"));
    }
