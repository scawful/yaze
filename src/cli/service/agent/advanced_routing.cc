...
AdvancedRouter::RoutedResponse AdvancedRouter::RouteHexAnalysis(
  ...
  // Summary for user
  response.summary = absl::StrFormat("Address 0x%06X contains %s (%zu bytes)",
  ...
  if (data_type.find("sprite") != std::string::npos) {
    response.next_steps =
        "Use resource-list --type=sprite to identify sprite IDs";
...
AdvancedRouter::RoutedResponse AdvancedRouter::RoutePaletteAnalysis(
  ...
  response.summary = absl::StrFormat("Palette has %zu colors (%d unique)",
...
AdvancedRouter::RoutedResponse AdvancedRouter::SynthesizeMultiToolResponse(
  ...
  response.summary =
      absl::StrFormat("Analyzed %zu data sources", tool_results.size());
