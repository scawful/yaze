...
std::optional<ChatMessage::TableData> BuildTableData(
  ...
  if (data.is_array()) {
    ...
    const bool all_objects = std::all_of(
        data.begin(), data.end(),
        [](const nlohmann::json& item) { return item.is_object(); });
...
ChatMessage::SessionMetrics ConversationalAgentService::BuildMetricsSnapshot()
    const {
  ...
  snapshot.total_elapsed_seconds =
      absl::ToDoubleSeconds(metrics_.total_latency);
...
absl::StatusOr<ChatMessage> ConversationalAgentService::SendMessage(
  ...
  if (config_.verbose) {
    util::PrintInfo(absl::StrCat("Starting agent loop (max ", max_iterations,
                                 " iterations)"));
    util::PrintInfo(
        absl::StrCat("History size: ", history_.size(), " messages"));
  }
  ...
  for (int iteration = 0; iteration < max_iterations; ++iteration) {
    ...
    util::LoadingIndicator loader(
        waiting_for_text_response ? "Generating final response..."
                                  : "Thinking...",
    ...
    if (!response_or.ok()) {
      util::PrintError(absl::StrCat("Failed to get AI response: ",
    ...
    if (config_.verbose) {
      ...
      if (!agent_response.reasoning.empty() && config_.show_reasoning) {
        std::cout << util::colors::kYellow
                  << "  💭 Reasoning: " << util::colors::kDim
                  << agent_response.reasoning << util::colors::kReset
                  << std::endl;
      }
    }
    ...
    if (!agent_response.tool_calls.empty()) {
      ...
      for (const auto& tool_call : agent_response.tool_calls) {
        ...
        if (!tool_result_or.ok()) {
          util::PrintError(absl::StrCat("Tool execution failed: ",
                                        tool_result_or.status().message()));
        ...
        if (!tool_output.empty()) {
          ...
          if (config_.verbose) {
            std::cout << util::colors::kDim
                      << "Tool output (truncated):" << util::colors::kReset
                      << std::endl;
          ...
          tool_result_msg.is_internal =
              true;  // Don't show this to the human user
    ...
    if (waiting_for_text_response && agent_response.text_response.empty() &&
      ...
      util::PrintWarning(
          absl::StrCat("LLM did not provide text_response after receiving tool "
                       "results (Iteration: ",
                       iteration + 1, "/", max_iterations, ")"));
    ...
    if (!agent_response.commands.empty()) {
      ...
      if (rom_context_ == nullptr) {
        ...
        proposal_status =
            absl::FailedPreconditionError("ROM context is not loaded");
        util::PrintWarning(
        ...
        if (!creation_or.ok()) {
          ...
          if (config_.verbose) {
            util::PrintSuccess(absl::StrCat(
                "Created proposal ", proposal_result->metadata.id, " with ",
                proposal_result->change_count, " change(s)."));
          }
        }
      }
    }
    ...
    if (proposal_result.has_value()) {
      ...
      response_text.append(
          absl::StrCat("⚠️ Failed to prepare a proposal automatically: ",
                       proposal_status.message()));
    }
...
std::string ConversationalAgentService::BuildEnhancedPrompt(
...
std::string ConversationalAgentService::InjectLearnedContext(
    const std::string& message) {
...
ChatMessage ConversationalAgentService::EnhanceResponse(
    const ChatMessage& response, const std::string& user_message) {
