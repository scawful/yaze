...
void Emulator::RunAudioFrame() {
  ...
  // The goal is to allow playback_speed_ to control both tempo and pitch (like a tape player).
  // This is achieved by:
  // 1. Applying playback_speed_ to the time_adder (frame timing). This slows down or speeds up
  //    how frequently SNES frames are processed relative to real-time.
  // 2. Adjusting the `samples_to_generate` (the number of audio samples requested from the DSP)
  //    inversely proportional to playback_speed_. This stretches or compresses the audio
  //    data generated by the DSP for each processed SNES frame.
  //
  // Example:
  // - If playback_speed_ = 0.5 (half speed):
  //   - The emulator processes frames at half the rate (30 SNES frames/sec).
  //   - For each SNES frame, we request twice the normal amount of audio samples (e.g., 1600 samples instead of 800).
  //   - This means the DSP's internal resampling loop (Dsp::GetSamples) will consume source samples more slowly,
  //     resulting in lower pitch and slower tempo.
  //   - The audio backend is still fed samples at ~48kHz, keeping the buffer full and smooth.
  // - If playback_speed_ = 2.0 (double speed):
  //   - The emulator processes frames at double the rate (120 SNES frames/sec).
  //   - For each SNES frame, we request half the normal amount of audio samples (e.g., 400 samples instead of 800).
  //   - This makes the DSP's internal resampling loop consume source samples more quickly,
  //     resulting in higher pitch and faster tempo.
  //   - The audio backend is still fed samples at ~48kHz.
  // This approach provides smooth varispeed (pitch-shifted) playback without stuttering.
  ...
  // Apply playback speed to timing (Varispeed support)
  time_adder += seconds * static_cast<double>(playback_speed_);
  ...
  // Local buffer for native sample path
  // Size calculation: 48kHz / 60fps = 800 samples/frame base.
  // At 0.25x speed (slowest), we generate 3200 samples/frame.
  // Stereo = 6400 samples. Add safety margin -> 8192.
  static int16_t native_audio_buffer[8192];
