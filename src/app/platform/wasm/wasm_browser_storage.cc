...
absl::Status WasmBrowserStorage::StoreApiKey(const std::string& service,
...
absl::StatusOr<std::string> WasmBrowserStorage::RetrieveApiKey(
...
absl::Status WasmBrowserStorage::ClearApiKey(const std::string& service,
                                             StorageType storage_type) {
  if (service.empty()) {
    return absl::InvalidArgumentError("Service name cannot be empty");
  }
...
bool WasmBrowserStorage::HasApiKey(const std::string& service,
                                   StorageType storage_type) {
  if (service.empty()) {
    return false;
  }
...
absl::Status WasmBrowserStorage::StoreSecret(const std::string& key,
...
absl::StatusOr<std::string> WasmBrowserStorage::RetrieveSecret(
    const std::string& key,
    StorageType storage_type) {
  if (key.empty()) {
    return absl::InvalidArgumentError("Key cannot be empty");
...
absl::Status WasmBrowserStorage::ClearSecret(const std::string& key,
                                             StorageType storage_type) {
  if (key.empty()) {
    return absl::InvalidArgumentError("Key cannot be empty");
  }
...
std::vector<std::string> WasmBrowserStorage::ListStoredApiKeys(
    StorageType storage_type) {
...
absl::Status WasmBrowserStorage::ClearAllApiKeys(StorageType storage_type) {
  if (storage_type == StorageType::kSession) {
...
bool WasmBrowserStorage::IsStorageAvailable() {
  return js_is_storage_available() != 0;
}

absl::StatusOr<WasmBrowserStorage::StorageQuota> WasmBrowserStorage::GetStorageQuota(
    StorageType storage_type) {
...
std::string WasmBrowserStorage::BuildApiKeyStorageKey(const std::string& service) {
  return absl::StrCat(kApiKeyPrefix, service);
}

std::string WasmBrowserStorage::BuildSecretStorageKey(const std::string& key) {
  return absl::StrCat(kSecretPrefix, key);
}

std::string WasmBrowserStorage::ExtractServiceFromKey(const std::string& storage_key) {
