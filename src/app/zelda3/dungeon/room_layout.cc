...
absl::Status RoomLayout::LoadLayout(int room_id) {
  ...
  if (layout_data_address < 0 || layout_data_address >= (int)rom_->size()) {
    return absl::OutOfRangeError(
        absl::StrFormat("Layout data address out of range: %#06x", layout_data_address));
  }
...
absl::Status RoomLayout::ParseLayoutData(const std::vector<uint8_t>& data) {
  ...
  // This is a simplified implementation - in reality, the format is more complex
...
RoomLayoutObject RoomLayout::CreateLayoutObject(int16_t tile_id, uint8_t x, uint8_t y, uint8_t layer) {
...
std::vector<RoomLayoutObject> RoomLayout::GetObjectsByType(RoomLayoutObject::Type type) const {
...
absl::StatusOr<RoomLayoutObject> RoomLayout::GetObjectAt(uint8_t x, uint8_t y, uint8_t layer) const {
  for (const auto& obj : objects_) {
