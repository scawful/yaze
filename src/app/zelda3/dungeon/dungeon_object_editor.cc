...
DungeonObjectEditor::DungeonObjectEditor(Rom* rom) 
    : rom_(rom)
    , renderer_(std::make_unique<UnifiedObjectRenderer>(rom))
    , config_{}
    , editing_state_{}
    , selection_state_{} {
...
absl::Status DungeonObjectEditor::ClearRoom() {
  ...
  // Clear all objects
  current_room_->tile_objects_.clear();
...
absl::Status DungeonObjectEditor::InsertObject(int x, int y, int object_type, int size, int layer) {
  ...
  if (config_.validate_objects) {
    for (const auto& existing_obj : current_room_->tile_objects_) {
  ...
  // Add object to room
  current_room_->tile_objects_.push_back(new_object);
  
  // Select the new object
  ClearSelection();
  selection_state_.selected_objects.push_back(current_room_->tile_objects_.size() - 1);
  
  // Notify callbacks
  if (object_changed_callback_) {
    object_changed_callback_(current_room_->tile_objects_.size() - 1, new_object);
  }
...
absl::Status DungeonObjectEditor::DeleteObject(size_t object_index) {
  ...
  if (object_index >= current_room_->tile_objects_.size()) {
  ...
  current_room_->tile_objects_.erase(current_room_->tile_objects_.begin() + object_index);
...
absl::Status DungeonObjectEditor::DeleteSelectedObjects() {
  ...
  for (size_t index : sorted_selection) {
    if (index < current_room_->tile_objects_.size()) {
      current_room_->tile_objects_.erase(current_room_->tile_objects_.begin() + index);
    }
  }
...
absl::Status DungeonObjectEditor::MoveObject(size_t object_index, int new_x, int new_y) {
  ...
  // Get the object
  auto& object = current_room_->tile_objects_[object_index];
  ...
  if (config_.validate_objects) {
    ...
    for (size_t i = 0; i < current_room_->tile_objects_.size(); i++) {
      if (i != object_index && ObjectsCollide(test_object, current_room_->tile_objects_[i])) {
...
std::optional<size_t> DungeonObjectEditor::FindObjectAt(int room_x, int room_y) {
  ...
  for (int i = static_cast<int>(current_room_->tile_objects_.size()) - 1; i >= 0; i--) {
    if (IsObjectAtPosition(current_room_->tile_objects_[i], room_x, room_y)) {
...
absl::Status DungeonObjectEditor::CreateUndoPoint() {
  ...
  undo_point.objects = current_room_->tile_objects_;
...
absl::Status DungeonObjectEditor::Undo() {
  ...
  current_state.objects = current_room_->tile_objects_;
...
absl::Status DungeonObjectEditor::ApplyUndoPoint(const UndoPoint& undo_point) {
  ...
  // Restore room state
  current_room_->tile_objects_ = undo_point.objects;
...
absl::Status DungeonObjectEditor::RenderRoom() {
  if (current_room_ == nullptr) {
    return absl::FailedPreconditionError("No room loaded");
  }
  ...
  // Render room objects
  auto result = renderer_->RenderObjects(current_room_->tile_objects_, palette);
  if (!result.ok()) {
