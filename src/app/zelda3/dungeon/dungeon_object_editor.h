...
class DungeonObjectEditor {
 public:
  ...
  int GetCurrentObjectType() const { return editing_state_.current_object_type; }
  ...
  size_t GetObjectCount() const { return current_room_ ? current_room_->tile_objects_.size() : 0; }
  const std::vector<RoomObject>& GetObjects() const { 
    return current_room_ ? current_room_->tile_objects_ : empty_objects_; 
  }
