...
class DungeonEditorSystem {
 public:
  ...
  EditorMode GetEditorMode() const { return editor_state_.current_mode; }
  
  // Room management
  absl::Status SetCurrentRoom(int room_id);
  int GetCurrentRoom() const { return editor_state_.current_room_id; }
  ...
  absl::StatusOr<std::vector<SpriteData>> GetSpritesByType(SpriteType type);
  ...
  absl::StatusOr<std::vector<ItemData>> GetItemsByType(ItemType type);
  ...
  absl::StatusOr<std::vector<EntranceData>> GetEntrancesByType(EntranceType type);
  ...
  // Getters
  EditorState GetEditorState() const { return editor_state_; }
  Rom* GetROM() const { return rom_; }
  bool IsDirty() const { return editor_state_.is_dirty; }
  bool HasUnsavedChanges() const { return editor_state_.is_dirty; }
...
std::vector<EntranceInfo> GetEntrancesByType(EntranceType type);
