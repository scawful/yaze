 ...
 public:
  ...
  absl::Status RunTests(TestResults& results) override {
    ...
    if (!current_rom) {
      ...
      rom_check_result.status = TestStatus::kSkipped;
      rom_check_result.error_message =
          absl::StrFormat("ROM not loaded (ptr: %p, title: '%s', size: %zu)",
                          (void*)current_rom, current_rom->title().c_str(),
                          current_rom->size());
    } else {
    ...
    if (test_manager.IsTestEnabled("ROM_Header_Validation_Test")) {
      ...
      AddSkippedTest(results, "ROM_Header_Validation_Test",
    ...
    if (test_manager.IsTestEnabled("ROM_Graphics_Extraction_Test")) {
      ...
      AddSkippedTest(results, "ROM_Graphics_Extraction_Test",
                     "Test disabled by user");
    }
    ...
    if (test_manager.IsTestEnabled("ROM_Overworld_Loading_Test")) {
      ...
      AddSkippedTest(results, "ROM_Overworld_Loading_Test",
                     "Test disabled by user");
    }
    ...
    if (test_manager.IsTestEnabled("Comprehensive_Save_Test")) {
      ...
      AddSkippedTest(results, "Comprehensive_Save_Test",
  ...
  void RunRomHeaderValidationTest(TestResults& results, Rom* rom) {
    ...
    if (!test_header_validation_) {
      ...
      try {
        ...
        // Basic validation
        bool valid_title =
            !title.empty() && title != "ZELDA3" && title.length() <= 21;
        bool valid_size =
            size >= 1024 * 1024 && size <= 8 * 1024 * 1024;  // 1MB to 8MB
        ...
        if (valid_title && valid_size) {
          ...
          result.error_message = absl::StrFormat(
              "ROM header validation failed: title='%s' size=%zu",
              title.c_str(), size);
        }
  ...
  void RunTile16EditorTest(TestResults& results, Rom* rom) {
    ...
    try {
      ...
      if (load_status.ok() && game_data.palette_groups.overworld_main.size() > 0) {
        ...
        std::vector<uint8_t> test_blockset_data(256 * 8192,
                                                1);  // Tile16 blockset size
        ...
        // Test initialization
        auto init_status = tile16_editor.Initialize(test_blockset_bmp,
                                                    test_gfx_bmp, tile_types);
        if (!init_status.ok()) {
          result.status = TestStatus::kFailed;
          result.error_message =
    ...
    try {
      ...
      auto test_status =
          test_manager.TestRomWithCopy(rom, [&](Rom* test_rom) -> absl::Status {
            ...
            test_map->set_area_graphics(
                0x01);  // Change to a different graphics set
            ...
            settings.filename =
                test_manager.GenerateTestRomFilename(test_rom->title());
