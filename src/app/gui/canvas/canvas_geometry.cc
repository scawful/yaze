...
CanvasGeometry CalculateCanvasGeometry(const CanvasConfig& config,
  ...
  geometry.scaled_size =
      CanvasUtils::CalculateScaledCanvasSize(canvas_sz, config.global_scale);
  ...
  geometry.canvas_p1 = ImVec2(geometry.canvas_p0.x + geometry.scaled_size.x,
...
ImVec2 CalculateMouseInCanvas(const CanvasGeometry& geometry,
  ...
  return ImVec2(mouse_screen_pos.x - origin.x, mouse_screen_pos.y - origin.y);
}
