...
void RenderMenuItem(const CanvasMenuItem& item,
  ...
  if (item.subitems.empty()) {
    ...
    if (selected) {
      ...
      if (item.popup.has_value() && 
          item.popup->auto_open_on_select && 
          popup_opened_callback) {
        popup_opened_callback(item.popup->popup_id, item.popup->render_callback);
      }
    }
  } else {
    // Submenu
