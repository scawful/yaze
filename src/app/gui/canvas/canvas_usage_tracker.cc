...
void CanvasUsageTracker::UpdateCanvasState(const ImVec2& canvas_size,
                                           ...
                                           float global_scale, float grid_step,
                                           bool enable_grid,
...
std::string CanvasUsageTracker::ExportUsageReport() const {
  ...
  report << "  Content Size: "
         << static_cast<int>(current_stats_.content_size.x) << " x "
         << static_cast<int>(current_stats_.content_size.y) << "\n";
  ...
  report << "  Grid Enabled: " << (current_stats_.enable_grid ? "Yes" : "No")
         << "\n";
  report << "  Hex Labels: "
         << (current_stats_.enable_hex_labels ? "Yes" : "No") << "\n";
  report << "  Custom Labels: "
         << (current_stats_.enable_custom_labels ? "Yes" : "No") << "\n\n";
...
void CanvasUsageTracker::EndSession() {
  ...
  LOG_DEBUG(
      "CanvasUsage", "Canvas %s: Session ended. Duration: %s, Operations: %d",
      canvas_id_.c_str(),
...
void CanvasUsageTracker::SaveCurrentStats() {
  ...
  current_stats_.total_time =
      current_stats_.active_time + current_stats_.idle_time;
...
double CanvasUsageTracker::CalculateAverageOperationTime(
...
std::string CanvasUsageTracker::FormatDuration(
...
void CanvasUsageManager::RegisterTracker(
    const std::string& canvas_id, std::shared_ptr<CanvasUsageTracker> tracker) {
  trackers_[canvas_id] = tracker;
  LOG_DEBUG("CanvasUsage", "Registered usage tracker for canvas: %s",
            canvas_id.c_str());
}

std::shared_ptr<CanvasUsageTracker> CanvasUsageManager::GetTracker(
...
CanvasUsageStats CanvasUsageManager::GetGlobalStats() const {
  ...
  for (const auto& [id, tracker] : trackers_) {
    ...
    // Update averages
    if (stats.average_operation_time_ms >
        global_stats.average_operation_time_ms) {
