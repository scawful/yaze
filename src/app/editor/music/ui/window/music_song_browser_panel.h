...
class MusicSongBrowserPanel : public WindowContent {
 public:
  ...
  void SetOnSongSelected(std::function<void(int)> callback) {
    if (song_browser_view_)
      song_browser_view_->SetOnSongSelected(callback);
  }

  void SetOnOpenTracker(std::function<void(int)> callback) {
    if (song_browser_view_)
      song_browser_view_->SetOnOpenTracker(callback);
  }

  void SetOnOpenPianoRoll(std::function<void(int)> callback) {
    if (song_browser_view_)
      song_browser_view_->SetOnOpenPianoRoll(callback);
  }

  void SetOnExportAsm(std::function<void(int)> callback) {
    if (song_browser_view_)
      song_browser_view_->SetOnExportAsm(callback);
  }

  void SetOnImportAsm(std::function<void(int)> callback) {
    if (song_browser_view_)
      song_browser_view_->SetOnImportAsm(callback);
  }

  void SetOnEdit(std::function<void()> callback) {
    if (song_browser_view_)
      song_browser_view_->SetOnEdit(callback);
  }
