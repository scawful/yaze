...
class MusicPlaybackControlPanel : public WindowContent {
 ...
 private:
  ...
  void DrawDebugControls() {
    ...
    if (!ImGui::CollapsingHeader(ICON_MD_BUG_REPORT " Debug Controls"))
      return;
    ...
    if (!debug_paused_) {
      ...
      if (cached_audio_.is_playing) {
        ...
        if (elapsed >= 0.5) {
          ...
          int32_t queue_delta =
              static_cast<int32_t>(cached_audio_.queued_frames) -
    ...
    ImGui::Separator();
    ImVec4 status_color = cached_audio_.is_playing ? gui::GetSuccessColor()
    ...
    if (ImGui::TreeNode("DSP Buffer")) {
      ...
      if (dsp.echo_enabled) {
        ImGui::TextColored(gui::GetInfoColor(),
                           ICON_MD_SURROUND_SOUND " Echo (delay=%u)",
                           dsp.echo_delay);
      }
    ...
    if (ImGui::TreeNode("Audio Queue")) {
      ...
      ImVec4 queue_color =
          (queue_level < 0.2f) ? gui::GetErrorColor() : gui::GetSuccessColor();
      {
    ...
    if (ImGui::TreeNode("APU Timing")) {
      ...
      if (ImGui::BeginTable("Timers", 4, ImGuiTableFlags_Borders)) {
        ...
        ImGui::TableNextColumn();
        ImGui::Text("%u", apu.timer0_counter);
        ImGui::TableNextColumn();
        ImGui::Text("%u", apu.timer0_target);
        ...
        ImGui::TableNextColumn();
        ImGui::Text("%u", apu.timer1_counter);
        ImGui::TableNextColumn();
        ImGui::Text("%u", apu.timer1_target);
        ...
        ImGui::TableNextColumn();
        ImGui::Text("%u", apu.timer2_counter);
        ImGui::TableNextColumn();
        ImGui::Text("%u", apu.timer2_target);
    ...
    if (ImGui::TreeNode("Channels")) {
      ...
      if (ImGui::BeginTable("ChannelDetails", 6,
        ...
        for (int i = 0; i < 8; i++) {
          ...
          ImGui::TableNextColumn();
          ImGui::Text("%02X", channels[i].sample_index);
          ImGui::TableNextColumn();
          ImGui::Text("%04X", channels[i].pitch);
          ImGui::TableNextColumn();
