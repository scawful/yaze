...
std::string DescribeCommand(uint8_t opcode) {
  switch (opcode) {
    case 0xE0: return "Set Instrument";
    case 0xE1: return "Set Pan";
    case 0xE5: return "Master Volume";
    case 0xE6: return "Master Volume Fade";
    case 0xE7: return "Set Tempo";
    case 0xE8: return "Tempo Fade";
    case 0xE9: return "Global Transpose";
    case 0xEA: return "Channel Transpose";
    case 0xEB: return "Tremolo On";
    case 0xEC: return "Tremolo Off";
    case 0xED: return "Channel Volume";
    case 0xEE: return "Channel Volume Fade";
    case 0xEF: return "Call Subroutine";
    case 0xF0: return "Vibrato Fade";
    case 0xF1: return "Pitch Env To";
    case 0xF2: return "Pitch Env From";
    case 0xF3: return "Pitch Env Off";
    case 0xF4: return "Tuning";
    case 0xF5: return "Echo Bits";
    case 0xF6: return "Echo Off";
    case 0xF7: return "Echo Params";
    case 0xF8: return "Echo Vol Fade";
    case 0xF9: return "Pitch Slide";
    case 0xFA: return "Percussion Patch";
...
constexpr CommandOption kCommandOptions[] = {
    ...
    {"Vibrato Off", 0xE4},
    {"Master Volume", 0xE5},
    {"Master Volume Fade", 0xE6},
    {"Set Tempo", 0xE7},
    {"Tempo Fade", 0xE8},
    {"Global Transpose", 0xE9},
    {"Channel Transpose", 0xEA},
    {"Tremolo On", 0xEB},
    {"Tremolo Off", 0xEC},
    {"Channel Volume", 0xED},
    {"Channel Volume Fade", 0xEE},
    {"Call Subroutine", 0xEF},
    {"Vibrato Fade", 0xF0},
    {"Pitch Env To", 0xF1},
    {"Pitch Env From", 0xF2},
    {"Pitch Env Off", 0xF3},
  ...
  if (ImGui::BeginTable("TrackerTable", 9, 
    ...
    while (clipper.Step()) {
      for (int row = clipper.DisplayStart; row < clipper.DisplayEnd; ++row) {
        ...
        if (row % 4 == 0) {
          ImGui::TableSetBgColor(ImGuiTableBgTarget_RowBg0, GetColorBeatHighlight());
        }
        ...
        for (int ch = 0; ch < 8; ++ch) {
          ...
          // Selection drawing
          bool is_selected = (row == selected_row_ && (ch + 1) == selected_col_);
          ...
          if (selection_anchor_row_ != -1 && selection_anchor_col_ != -1) {
            ...
            if (row >= row_start && row <= row_end && (ch + 1) >= col_start && (ch + 1) <= col_end) {
...
void TrackerView::DrawEventCell(MusicTrack& track, int event_index, int channel_idx, uint16_t tick, const MusicBank* bank) {
  TrackEvent* event_ptr = (event_index >= 0 && event_index < static_cast<int>(track.events.size()))
  ...
  if (!has_event) {
    ...
    switch (event.type) {
      case TrackEvent::Type::Note:
        ImGui::TextColored(ImColor(GetColorNote()), "%s", event.note.GetNoteName().c_str());
        ...
        if (event.command.opcode == 0xE0) { // Set Instrument
          if (bank) {
            ...
            if (inst) {
              ...
              tooltip = absl::StrFormat("Set Instrument: %s (ID %02X)", inst->name.c_str(), event.command.params[0]);
            } else {
          ...
          if (pan == 0x0A) label = "Pan: Center";
          else if (pan < 0x0A) label = absl::StrFormat("Pan: L%d", 0x0A - pan);
          else label = absl::StrFormat("Pan: R%d", pan - 0x0A);
    ...
    if (event.type == TrackEvent::Type::Note) {
      ...
      if (note_labels.empty()) {
        for (int p = kNoteMinPitch; p <= kNoteMaxPitch; ++p) {
          Note n; n.pitch = static_cast<uint8_t>(p);
      ...
      if (opcode == 0xE0 && bank) { // Set Instrument
        ...
        std::string preview = inst ? absl::StrFormat("%02X: %s", p0, inst->name.c_str()) : absl::StrFormat("%02X", p0);
        ...
        ImGui::InputInt("Param 0 (hex)", &p0, 1, 4, ImGuiInputTextFlags_CharsHexadecimal);
      }
      
      ImGui::InputInt("Param 1 (hex)", &p1, 1, 4, ImGuiInputTextFlags_CharsHexadecimal);
      ImGui::InputInt("Param 2 (hex)", &p2, 1, 4, ImGuiInputTextFlags_CharsHexadecimal);
...
void TrackerView::HandleKeyboardInput(MusicSong* song) {
  if (!song || song->segments.empty()) return;
  
  if (selected_row_ < 0 || selected_col_ < 1 || selected_col_ > 8) return;
  ...
  struct KeyNote { ImGuiKey key; int semitone; int octave_offset; };
  static const KeyNote key_map[] = {
    {ImGuiKey_Z, 0, 0}, {ImGuiKey_S, 1, 0}, {ImGuiKey_X, 2, 0}, {ImGuiKey_D, 3, 0},
    {ImGuiKey_C, 4, 0}, {ImGuiKey_V, 5, 0}, {ImGuiKey_G, 6, 0}, {ImGuiKey_B, 7, 0},
    {ImGuiKey_H, 8, 0}, {ImGuiKey_N, 9, 0}, {ImGuiKey_J, 10, 0}, {ImGuiKey_M, 11, 0},
    {ImGuiKey_Q, 0, 1}, {ImGuiKey_2, 1, 1}, {ImGuiKey_W, 2, 1}, {ImGuiKey_3, 3, 1},
    {ImGuiKey_E, 4, 1}, {ImGuiKey_R, 5, 1}, {ImGuiKey_5, 6, 1}, {ImGuiKey_T, 7, 1},
    {ImGuiKey_6, 8, 1}, {ImGuiKey_Y, 9, 1}, {ImGuiKey_7, 10, 1}, {ImGuiKey_U, 11, 1}
  };

  static int base_octave = 4; // Default octave

  // Octave Control
  if (ImGui::IsKeyPressed(ImGuiKey_F1)) base_octave = std::max(1, base_octave - 1);
  if (ImGui::IsKeyPressed(ImGuiKey_F2)) base_octave = std::min(6, base_octave + 1);
  ...
  for (const auto& kn : key_map) {
    if (ImGui::IsKeyPressed(kn.key)) {
      ...
      if (!found) {
        track.InsertEvent(TrackEvent::MakeNote(tick, pitch, kDurationSixteenth));
      }
...
void TrackerView::HandleEditShortcuts(MusicSong* song) {
  ...
  if (ImGui::IsKeyPressed(ImGuiKey_I) && (ImGui::GetIO().KeyCtrl || ImGui::GetIO().KeySuper)) {
    TriggerEdit();
  ...
  // Insert SetPan (Cmd+P)
  if (ImGui::IsKeyPressed(ImGuiKey_P) && (ImGui::GetIO().KeyCtrl || ImGui::GetIO().KeySuper)) {
    TriggerEdit();
  ...
  if (ImGui::IsKeyPressed(ImGuiKey_V) && (ImGui::GetIO().KeyCtrl || ImGui::GetIO().KeySuper)) {
    TriggerEdit();
