#include "app/editor/overworld/ui/items/overworld_item_list_view.h"
...
void OverworldItemListView::Draw(bool* p_open) {
  (void)p_open;
  const auto ctx = CurrentOverworldWindowContext();
  if (!ctx) {
    return;
  }
  auto* ow_editor = ctx.editor;
...
REGISTER_PANEL(OverworldItemListView);
