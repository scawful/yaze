#include "app/editor/graphics/ui/research/polyhedral_editor_view.h"
...
uint32_t PolyhedralEditorView::TablePc() const {
  return SnesToPc(kPolyTableSnes);
}

absl::Status PolyhedralEditorView::Load() {
  RETURN_IF_ERROR(LoadShapes());
  dirty_ = false;
  return absl::OkStatus();
}

absl::Status PolyhedralEditorView::LoadShapes() {
...
absl::Status PolyhedralEditorView::SaveShapes() {
...
absl::Status PolyhedralEditorView::WriteShape(const PolyShape& shape) {
  // Vertices
...
void PolyhedralEditorView::Draw(bool* p_open) {
...
absl::Status PolyhedralEditorView::Update() {
  if (!rom_ || !rom_->is_loaded()) {
    ImGui::TextUnformatted("Load a ROM to edit 3D objects.");
    return absl::OkStatus();
  }
...
void PolyhedralEditorView::DrawShapeEditor(PolyShape& shape) {
...
void PolyhedralEditorView::DrawVertexList(PolyShape& shape) {
...
void PolyhedralEditorView::DrawFaceList(PolyShape& shape) {
...
void PolyhedralEditorView::DrawPlot(const char* label, PlotPlane plane,
                                    PolyShape& shape) {
  if (shape.vertices.empty()) {
    return;
  }
...
void PolyhedralEditorView::DrawPreview(PolyShape& shape) {
