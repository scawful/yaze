#include "app/editor/graphics/ui/sprite/link_sprite_view.h"
...
LinkSpriteView::LinkSpriteView(GraphicsEditorState* state, Rom* rom)
    : state_(state), rom_(rom) {}

void LinkSpriteView::Initialize() {
  preview_canvas_.SetCanvasSize(
      ImVec2(128 * preview_zoom_, 32 * preview_zoom_));
}

void LinkSpriteView::Draw(bool* p_open) {
...
absl::Status LinkSpriteView::Update() {
  // Lazy-load Link sheets on first update
  if (!sheets_loaded_ && rom_ && rom_->is_loaded()) {
    auto status = LoadLinkSheets();
    if (!status.ok()) {
...
void LinkSpriteView::DrawToolbar() {
...
void LinkSpriteView::DrawSheetGrid() {
...
void LinkSpriteView::DrawSheetThumbnail(int sheet_index) {
...
void LinkSpriteView::DrawPreviewCanvas() {
...
void LinkSpriteView::DrawPaletteSelector() {
...
void LinkSpriteView::DrawInfoPanel() {
  ImGui::Text("Info:");
...
void LinkSpriteView::ImportZspr() {
  ...
  LOG_INFO("LinkSpriteView", "Importing ZSPR: %s", file_path.c_str());

  // Load ZSPR file
  auto zspr_result = gfx::ZsprLoader::LoadFromFile(file_path);
  if (!zspr_result.ok()) {
    LOG_ERROR("LinkSpriteView", "Failed to load ZSPR: %s",
  ...
  if (!loaded_zspr_->is_link_sprite()) {
    LOG_ERROR("LinkSpriteView", "ZSPR is not a Link sprite (type=%d)",
  ...
  if (rom_ && rom_->is_loaded()) {
    ...
    if (!status.ok()) {
      LOG_ERROR("LinkSpriteView", "Failed to apply ZSPR to ROM: %s",
                status.message().data());
      return;
    }
    ...
    if (!status.ok()) {
      LOG_WARN("LinkSpriteView", "Failed to apply ZSPR palette: %s",
               status.message().data());
    }
    ...
    LOG_INFO("LinkSpriteView", "ZSPR '%s' imported successfully",
             loaded_zspr_->metadata.display_name.c_str());
  }
}

void LinkSpriteView::ResetToVanilla() {
  ...
  LOG_WARN("LinkSpriteView", "Reset to vanilla not yet implemented");
  loaded_zspr_.reset();
}

void LinkSpriteView::OpenSheetInPixelEditor() {
  ...
  LOG_INFO("LinkSpriteView", "Request to open Link sheet %d in pixel editor",
           selected_sheet_);
...
absl::Status LinkSpriteView::LoadLinkSheets() {
  if (!rom_ || !rom_->is_loaded()) {
  ...
  LOG_INFO("LinkSpriteView", "Loaded %d Link graphics sheets",
...
void LinkSpriteView::ApplySelectedPalette() {
  if (!rom_ || !rom_->is_loaded())
    return;
  ...
  // Ideally, LinkSpriteView should have access to GameData.
  ...
  LOG_INFO("LinkSpriteView", "Applied palette %s to %zu sheets",
           GetPaletteName(selected_palette_), link_sheets_.size());
}

const char* LinkSpriteView::GetPaletteName(PaletteType type) {
  switch (type) {
