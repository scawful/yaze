#include "app/editor/graphics/ui/editing/pixel_editor_view.h"
...
void PixelEditorView::Initialize() {
  // Canvas is initialized via member initializer list
}

void PixelEditorView::Draw(bool* p_open) {
  // WindowContent interface - delegate to Update()
  Update().IgnoreError();
}

absl::Status PixelEditorView::Update() {
  // Top toolbar
  DrawToolbar();
  ImGui::SameLine();
  DrawViewControls();
...
void PixelEditorView::DrawToolbar() {
...
void PixelEditorView::DrawViewControls() {
  // Zoom controls
...
void PixelEditorView::DrawTransparencyGrid(float canvas_width,
                                           float canvas_height) {
...
void PixelEditorView::DrawCursorCrosshair() {
...
void PixelEditorView::DrawBrushPreview() {
  ImVec2 cursor_screen = PixelToScreen(cursor_x_, cursor_y_);
  float pixel_size = state_->zoom_level;
  int brush = state_->brush_size;
  int half = brush / 2;
...
void PixelEditorView::DrawPixelInfoTooltip(const gfx::Bitmap& sheet) {
...
void PixelEditorView::DrawTileHighlight(const gfx::Bitmap& sheet) {
...
void PixelEditorView::DrawColorPicker() {
  ImGui::Text("Colors");
...
void PixelEditorView::DrawMiniMap() {
...
void PixelEditorView::DrawStatusBar() {
  ImGui::Separator();
...
void PixelEditorView::HandleCanvasInput() {
  if (!ImGui::IsItemHovered()) {
...
void PixelEditorView::ApplyPencil(int x, int y) {
  auto& sheet =
      gfx::Arena::Get().mutable_gfx_sheets()->at(state_->current_sheet_id);
...
void PixelEditorView::ApplyBrush(int x, int y) {
  auto& sheet =
      gfx::Arena::Get().mutable_gfx_sheets()->at(state_->current_sheet_id);
...
void PixelEditorView::ApplyEraser(int x, int y) {
  auto& sheet =
      gfx::Arena::Get().mutable_gfx_sheets()->at(state_->current_sheet_id);
  int size = state_->brush_size;
  int half = size / 2;
...
void PixelEditorView::ApplyFill(int x, int y) {
  auto& sheet =
      gfx::Arena::Get().mutable_gfx_sheets()->at(state_->current_sheet_id);
...
void PixelEditorView::ApplyEyedropper(int x, int y) {
...
void PixelEditorView::DrawLine(int x1, int y1, int x2, int y2) {
  auto& sheet =
      gfx::Arena::Get().mutable_gfx_sheets()->at(state_->current_sheet_id);
...
void PixelEditorView::DrawRectangle(int x1, int y1, int x2, int y2,
                                    bool filled) {
  auto& sheet =
      gfx::Arena::Get().mutable_gfx_sheets()->at(state_->current_sheet_id);
...
void PixelEditorView::BeginSelection(int x, int y) {
...
void PixelEditorView::UpdateSelection(int x, int y) {
...
void PixelEditorView::EndSelection() {
...
void PixelEditorView::CopySelection() {
  // Selection data is already in state_->selection
}

void PixelEditorView::PasteSelection(int x, int y) {
...
void PixelEditorView::FlipSelectionHorizontal() {
  if (state_->selection.pixel_data.empty())
    return;
...
void PixelEditorView::FlipSelectionVertical() {
  if (state_->selection.pixel_data.empty())
    return;
...
void PixelEditorView::SaveUndoState() {
  if (!undo_manager_)
    return;
  auto& sheet = gfx::Arena::Get().gfx_sheets()[state_->current_sheet_id];
...
void PixelEditorView::FinalizeUndoAction() {
