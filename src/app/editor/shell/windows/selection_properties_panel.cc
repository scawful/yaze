...
std::string SelectionPropertiesPanel::BuildAgentPrompt(
...
void SelectionPropertiesPanel::DrawDungeonRoomProperties() {
  ...
  if (ImGui::CollapsingHeader("Graphics & Layout",
    ...
    if (DrawByteProperty("Spriteset", &spriteset,
  ...
  if (show_advanced_ &&
    ...
    auto draw_tag_combo = [&](const char* label, zelda3::TagKey current,
                              std::function<void(zelda3::TagKey)> setter) {
      std::string current_label =
          zelda3::GetRoomTagLabel(static_cast<int>(current));
    ...
    draw_tag_combo("Tag 1", room->tag1(),
                   [&](zelda3::TagKey val) { room->SetTag1(val); });
    draw_tag_combo("Tag 2", room->tag2(),
