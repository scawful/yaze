...
void FeatureFlagEditorPanel::Draw() {
...

  {
    ...
    if (ImGui::Button(
            absl::StrFormat(
                ICON_MD_SAVE " Save%s",
                dirty_count > 0 ? absl::StrFormat(" (%d)", dirty_count) : "")
                .c_str())) {
      if (SaveToFile()) {
  ...
  if (ImGui::BeginTable("FeatureFlagsTable", 5, table_flags)) {
    ...
    for (int i = 0; i < static_cast<int>(flags_.size()); ++i) {
      ...
      // Apply filter
      if (!filter.empty() && !ContainsCaseInsensitive(flag.name, filter) &&
...
bool FeatureFlagEditorPanel::SaveToFile() {
  ...
  if (!std::filesystem::exists(parent)) {
    status_message_ =
        absl::StrFormat("Config directory does not exist: %s", parent.string());
    return false;
  }
