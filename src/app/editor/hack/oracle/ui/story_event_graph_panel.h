#ifndef YAZE_APP_EDITOR_HACK_ORACLE_UI_STORY_EVENT_GRAPH_PANEL_H
#define YAZE_APP_EDITOR_HACK_ORACLE_UI_STORY_EVENT_GRAPH_PANEL_H
    ...
    if (!node->locations.empty()) {
      ...
      for (size_t i = 0; i < node->locations.size(); ++i) {
        ...
        if (jump_map_id) {
          ...
          ImGui::SameLine();
          ImGui::BeginDisabled();
          ImGui::SmallButton(tr("Map"));
          ImGui::EndDisabled();
          if (ImGui::IsItemHovered()) {
            ImGui::SetTooltip(
                tr("Map target is outside the supported overworld range."));
          }
        }
        ...
        if (!loc.room_id.empty() || !loc.overworld_id.empty() ||
          ...
          ImGui::TextDisabled(
              tr("room=%s  map=%s  special=%s  entrance=%s"),
...
#endif  // YAZE_APP_EDITOR_HACK_ORACLE_UI_STORY_EVENT_GRAPH_PANEL_H
