...
class ItemEditorPanel : public WindowContent {
 public:
  ItemEditorPanel(int* current_room_id, DungeonRoomStore* rooms,
 ...
 private:
  ...
  void DrawPlacementControls() {
    ...
    if (placement_mode_) {
      ...
      ImGui::TextColored(theme.status_warning,
                         ICON_MD_PLACE " Placing: %s (0x%02X)", item_name,
                         selected_item_id_);
  ...
  void DrawItemSelector() {
    ...
    float item_size =
        std::max(36.0f, std::min(48.0f, (panel_width - 40.0f) / 6.0f));
    int items_per_row =
        std::max(1, static_cast<int>(panel_width / (item_size + 8)));
