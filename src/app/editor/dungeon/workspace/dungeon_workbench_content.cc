...
void DungeonWorkbenchContent::DrawSidebarHeader(float button_size,
  ...
  if (can_open_overview) {
    ...
    if (ImGui::IsItemHovered()) {
      ImGui::SetTooltip(tr("Open room review tools"));
    }
  ...
  if (ImGui::IsItemHovered()) {
    ImGui::SetTooltip(tr("Collapse navigation pane"));
  }
...
void DungeonWorkbenchContent::Draw(bool* p_open) {
  ...
  if (show_left) {
    DrawSidebarPane(left_w, total_h, btn, responsive.compact_left);
  }
  if (show_left) {
  ...
  if (show_right) {
    DrawInspectorPane(right_w, total_h, btn, responsive.compact_right,
                      primary_viewer);
  }
}
...
void DungeonWorkbenchContent::DrawInspectorHeader(float button_size,
  ...
  if (ImGui::IsItemHovered()) {
    ImGui::SetTooltip(tr("Collapse inspector"));
  }
...
void DungeonWorkbenchContent::DrawInspectorCompactSummary(
  ...
  if (room_id >= 0) {
    ImGui::Spacing();
    if (on_save_room_ &&
        DrawWorkbenchActionButton(ICON_MD_SAVE " Save Room", ImVec2(-1, 0))) {
...
void DungeonWorkbenchContent::DrawInspectorShelfRoom(
  ...
  if (on_save_room_ && room_id >= 0) {
    if (DrawWorkbenchActionButton(ICON_MD_SAVE " Save Room", ImVec2(-1, 0))) {
      on_save_room_(room_id);
    }
  }
  ...
  if (auto* rooms = viewer.rooms();
    ...
    if (ImGui::BeginTable("##WorkbenchRoomProps", 2, kPropsFlags)) {
      ...
      gui::LayoutHelpers::PropertyRow("Blockset", [&]() {
        ...
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(tr("Blockset (0-51)"));
        }
      });
      gui::LayoutHelpers::PropertyRow("Palette", [&]() {
        ...
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(tr("Palette (0-47)"));
        }
      });
      gui::LayoutHelpers::PropertyRow("Layout", [&]() {
        ...
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(tr("Layout (0-7)"));
        }
      });
      gui::LayoutHelpers::PropertyRow("Spriteset", [&]() {
        ...
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(tr("Spriteset (0-8F)"));
        }
      });
...
void DungeonWorkbenchContent::DrawInspectorShelfSelection(
  ...
  if (obj_count > 0) {
    ...
    if (ImGui::BeginTable("##SelectionObjectActions", 3, kActionFlags)) {
      ...
      ImGui::TableNextColumn();
      if (DrawWorkbenchActionButton(ICON_MD_CLEAR " Clear", ImVec2(-1, 0))) {
  ...
  if (has_entity && room_id >= 0 && viewer.rooms()) {
    ...
    if (ImGui::BeginTable(
      ...
      ImGui::TableNextRow();
      ImGui::TableNextColumn();
      if (DrawWorkbenchActionButton(ICON_MD_DELETE " Delete Entity",
                                    ImVec2(-1, 0))) {
      ...
      if (show_panel_ && entity_panel_id && entity_action_label) {
        if (DrawWorkbenchActionButton(entity_action_label, ImVec2(-1, 0))) {
          show_panel_(entity_panel_id);
        }
      } else {
        ImGui::BeginDisabled();
        DrawWorkbenchActionButton(ICON_MD_OPEN_IN_NEW " Open Panel",
                                  ImVec2(-1, 0));
        ImGui::EndDisabled();
      }
      ImGui::EndTable();
...
void DungeonWorkbenchContent::DrawInspectorShelfTools(
    DungeonCanvasViewer& /*viewer*/) {
  if (!show_panel_) {
    ImGui::TextDisabled(tr("No panel launcher available"));
    return;
  }
