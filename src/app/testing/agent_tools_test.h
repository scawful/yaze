 ...
 private:
  ...
  void RunConfigPersistenceTest(TestResults& results) {
    ...
    std::filesystem::path temp_path =
        std::filesystem::temp_directory_path() / "yaze_test_config.json";
    auto save_status =
    ...
    if (!save_status.ok()) {
      ...
      result.error_message =
          "Save failed: " + std::string(save_status.message());
    } else {
      // Load back
      ...
      if (load_result.ok()) {
        if (load_result->agent_config.has_value() &&
          ...
          yaze::editor::AgentChatHistoryCodec::Save(temp_path, snapshot);
          auto load_result2 =
              yaze::editor::AgentChatHistoryCodec::Load(temp_path);

          if (load_result2.ok() && load_result2->agent_config.has_value() &&
        ...
        result.status = TestStatus::kFailed;
        result.error_message =
            "Load failed: " + std::string(load_result.status().message());
      }
    }
    // Cleanup
    std::error_code ec;
