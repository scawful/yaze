    ...
    try {
      ...
      auto test_status =
          test_manager.TestRomWithCopy(rom, [&](Rom* test_rom) -> absl::Status {
            ...
            bool success =
                editor.AutomationSetTile(test_tile_x, test_tile_y, new_tile_id);
            if (!success) {
            ...
            int actual_tile =
                editor.AutomationGetTile(test_tile_x, test_tile_y);
            if (actual_tile != new_tile_id) {
              return absl::InternalError(
                  absl::StrFormat("Tile mismatch: expected 0x%04X, got 0x%04X",
                                  new_tile_id, actual_tile));
            }
    ...
    try {
      ...
      auto test_status =
          test_manager.TestRomWithCopy(rom, [&](Rom* test_rom) -> absl::Status {
            ...
            if (current_map_data->sprites().size() !=
