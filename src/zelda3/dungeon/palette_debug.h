...
struct PaletteDebugEvent {
  ...
  PaletteDebugLevel level = PaletteDebugLevel::INFO;
  int palette_id = -1;                 // Which palette
  int color_count = 0;                 // How many colors
  std::vector<uint8_t> sample_colors;  // RGB of first 3 colors for verification
  uint64_t timestamp_ms = 0;           // Timestamp for timeline analysis
  int sequence_number = 0;             // Order of events
};
...
class PaletteDebugger {
 public:
  ...
  void LogTextureCreation(const std::string& location, bool has_palette,
                          int color_count);
 ...
 private:
  ...
  void AddComparisonLimited(const ColorComparison& comp) {
    if (comparisons_.size() >= kMaxComparisons) {
      comparisons_.erase(comparisons_.begin(),
                         comparisons_.begin() + kMaxComparisons / 2);
    }
