...
const std::vector<ObjectCategory>& CategoryTable() {
  // Explicit memberships follow Type1/2/3RoomObjectNames in room_object.h.
  // These are browsing categories, not collision or gameplay metadata. Keep
  // names/custom labels out of classification so translations cannot change
  // which objects a filter exposes. Oracle dispatch families remain in the
  // selector's separate Custom Assets browser.
  static const std::vector<ObjectCategory> categories = [] {
    std::vector<ObjectCategory> result = {
        {"Walls",
         {0x000, 0x001, 0x002, 0x003, 0x004, 0x005, 0x006, 0x007, 0x008, 0x009,
          ...
          0x01E, 0x01F, 0x020, 0x02F, 0x030, 0x060, 0x061, 0x062, 0x063, 0x064,
          0x065, 0x066, 0x067, 0x068, 0x06C, 0x06D, 0x0A0, 0x0A1, 0x0A2, 0x0A3,
          0x0C0, 0x0CD, 0x0CE, 0x100, 0x101, 0x102, 0x103, 0x104, 0x105, 0x106,
          0x107, 0x108, 0x109, 0x10A, 0x10B, 0x10C, 0x10D, 0x10E, 0x10F, 0x110,
          0x111, 0x112, 0x113, 0x114, 0x115, 0x116, 0x117, 0x118, 0x119, 0x11A,
          0x11B, 0x13C, 0xFA2, 0xFA3, 0xFA4, 0xFA5},
         "Walls, wall corners, and ceilings"},
        {"Floors",
         {0x023, 0x024, 0x025, 0x026, 0x027, 0x028, 0x029, 0x02A, 0x02B,
          0x02C, 0x02D, 0x02E, 0x033, 0x034, 0x03F, 0x040, 0x041, 0x042,
          0x043, 0x044, 0x045, 0x046, 0x047, 0x048, 0x049, 0x04A, 0x06A,
          0x06B, 0x070, 0x071, 0x079, 0x07A, 0x08B, 0x08C, 0x08D, 0x08E,
          0x094, 0x0A4, 0x0B0, 0x0B1, 0x0B2, 0x0B3, 0x0B4, 0x0BA, 0x0C1,
          0x0C4, 0x0C5, 0x0C7, 0x0C8, 0x0C9, 0x0CA, 0x0D1, 0x0D2, 0x0D8,
          0x0DA, 0x0DB, 0x0DC, 0x0DF, 0x0E0, 0x0E1, 0x0E2, 0x0E3, 0x0E4,
          0x0E5, 0x0E6, 0x0E7, 0x0E8, 0xFC7, 0xFC8, 0xFE6, 0xFF8},
         "Floor surfaces, carpet, pits, water, ice, and conveyors"},
        {"Decorations",
         {0x036, 0x037, 0x038, 0x039, 0x03A, 0x03B, 0x03C, 0x03D, 0x03E, 0x04B,
          0x04C, 0x04D, 0x04E, 0x04F, 0x051, 0x052, 0x055, 0x056, 0x05B, 0x05C,
          0x073, 0x074, 0x075, 0x076, 0x077, 0x078, 0x07B, 0x07F, 0x080, 0x081,
          0x082, 0x083, 0x084, 0x085, 0x086, 0x087, 0x08F, 0x090, 0x091, 0x095,
          0x0B5, 0x0B6, 0x0B7, 0x0BB, 0x0BC, 0x0DD, 0x11C, 0x11D, 0x120, 0x121,
          0x122, 0x123, 0x124, 0x125, 0x126, 0x127, 0x128, 0x129, 0x12A, 0x12B,
          0x12C, 0x13D, 0x13E, 0x13F, 0xFAA, 0xFAB, 0xFAD, 0xFB0, 0xFB7, 0xFB8,
          0xFB9, 0xFC9, 0xFCB, 0xFCC, 0xFCD, 0xFCE, 0xFD1, 0xFD5, 0xFD6, 0xFD7,
          0xFD8, 0xFD9, 0xFDA, 0xFDB, 0xFDC, 0xFDD, 0xFDE, 0xFDF, 0xFE0, 0xFE1,
          0xFE3, 0xFEB, 0xFEC, 0xFED, 0xFEE, 0xFEF, 0xFF0, 0xFF1, 0xFF7, 0xFF9,
          0xFFA},
         "Furniture, ornaments, torches, and room scenery"},
        {"Interactive",
         {0x05E, 0x089, 0x092, 0x093, 0x096, 0x0B8, 0x0B9, 0x0BD,
          0x0DE, 0x11E, 0x11F, 0x134, 0x137, 0xF92, 0xF93, 0xF96,
          0xF98, 0xFAC, 0xFAF, 0xFCA, 0xFCF, 0xFD0, 0xFD2, 0xFD3},
         "Blocks, pegs, switches, locks, and other interaction tiles"},
        {"Stairs",
         ...
         "Platform, interroom, intraroom, water-hop stairs, and ladders"},
        {"Doors",
         {0x035, 0xFF4, 0xFF6},
         "Doorway tile objects; ordinary room doors use the Door Editor"},
        {"Chests",
         {0xF99, 0xF9A, 0xFB1, 0xFB2, 0xFF5},
         "Small, big, open, and minigame chest objects"},
        {"Special", {}, "Other, logic, unused, and unclassified tile objects"}};

    std::array<bool, 0x1000> categorized{};
    for (const auto& category : result) {
      for (const int id : category.object_ids) {
        categorized[id] = true;
      }
    }
    for (int id = 0; id < static_cast<int>(categorized.size()); ++id) {
      if (IsRepresentableRoomObjectId(id) && !categorized[id]) {
        result.back().object_ids.push_back(id);
      }
    }
    return result;
  }();
...
absl::StatusOr<std::vector<int>> GetObjectsInCategory(
  ...
  for (const auto& category : CategoryTable()) {
