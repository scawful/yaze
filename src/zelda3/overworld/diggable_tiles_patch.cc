...
std::string DiggableTilesPatch::GeneratePatch(
  ...
  asm_code << "; "
              "================================================================"
              "=============\n";
  asm_code << "; Replaces hardcoded diggable tile checks with bitfield table "
              "lookup.\n";
  asm_code
...
std::string DiggableTilesPatch::GenerateDataTable(
    const DiggableTiles& diggable_tiles, uint32_t table_address) {
  std::ostringstream asm_code;
...
absl::Status DiggableTilesPatch::ExportPatchFile(
    const DiggableTiles& diggable_tiles, const std::string& output_path,
    const DiggableTilesPatchConfig& config) {
...
DiggableTilesPatchConfig DiggableTilesPatch::GetRecommendedConfig(
  ...
  if (DetectZSDiggingHooks(rom)) {
    ...
    config.freespace_address =
        0x1BF800;  // Different location to avoid conflicts
  }
