...
std::vector<uint8_t> RenderTile16PixelsFromMetadata(
  ...
  for (int quadrant = 0; quadrant < 4; ++quadrant) {
    ...
    if (tile8_id < 0 || tile8_id >= static_cast<int>(tile8_pixels.size())) {
      continue;
    }

    const Tile8PixelData& source_tile8 = tile8_pixels[tile8_id];
    ...
    for (int ty = 0; ty < kTile8Size; ++ty) {
      for (int tx = 0; tx < kTile8Size; ++tx) {
        ...
        const uint8_t pixel = source_tile8[static_cast<size_t>(src_index)];
...
std::vector<uint8_t> RenderTile16PixelsFromMetadata(
  ...
  for (int quadrant = 0; quadrant < 4; ++quadrant) {
    ...
    if (!source_tile8.is_active() || source_tile8.data() == nullptr) {
      continue;
    }
...
absl::Status RenderTile16BitmapFromMetadata(
  ...
  if (tile8_pixels.empty()) {
    return absl::FailedPreconditionError("Tile8 bitmap source is empty");
  }

  const auto pixels = RenderTile16PixelsFromMetadata(tile_data, tile8_pixels);
