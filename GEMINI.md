# GEMINI.md - Build and Test Instructions for Gemini
...
1. **Always use the full preset name**: `mac-dbg` not just `dbg`
2. **First builds are LONG**: 10-20 minutes is normal, don't panic
3. **Incremental builds are FAST**: 10-60 seconds after first build
4. **Build specific targets**: `--target yaze` is faster than building everything
5. **Unit tests are fastest**: Use `--unit` flag for quick validation
6. **Check test output carefully**: Some tests require ROM files to pass

**Common Mistakes to Avoid**:

- ❌ Running `cmake .` (old style) → Use `cmake --preset mac-dbg`
- ❌ Forgetting to build tests before running them
- ❌ Running full builds when only one file changed
